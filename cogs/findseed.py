...
class Findseed(commands.Cog):
    ...
    @commands.command(
        aliases=["fsv", "findseedvisual", "visualfindseed"],
        description="Simulates Minecraft end portal generation with graphics",
    )
    @commands.cooldown(1, 60, commands.BucketType.guild)
    async def findseedbutvisual(self, ctx):
        ...
        output += f"{portals[11]}   {portals[3]}\n"
        output += f"{portals[10]}   {portals[4]}\n"
