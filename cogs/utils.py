...
class Utils(commands.Cog):
    ...
    async def update_activity(self):
        activity = (
            f"{self.bot.default_prefixes[0]}help | in {len(self.bot.guilds)} servers"
        )
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            await ctx.send(
                f"{ctx.author.mention}, you have to wait {_retry} seconds before using this again. The cooldown for this command is {_rate} per {_per}s for every {_type}"
            )
            return
