...
class Fun(commands.Cog):
    ...
    async def stats(self, ctx, name):
        try:
            ...
            embed = discord.Embed(
                title=f"Base stats for {pokemon.name}", colour=self.bot.main_colour
            )
    ...
    @commands.command(
        aliases=["random"],
        description="Generates a random number in the range minimum (inclusive) - maximum (inclusive)",
    )
    ...
    async def trivia(self, ctx):
        async with self.bot.session.get("http://jservice.io/api/random") as url:
    ...
    @commands.command(
        description='Get the "Astronomy Picture of the Day" from NASA', aliases=["apod"]
    )
